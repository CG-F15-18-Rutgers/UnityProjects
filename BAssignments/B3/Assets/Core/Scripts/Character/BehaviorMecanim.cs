...
public class BehaviorMecanim : MonoBehaviour
{
    ...
    /// <summary>
    /// Tries to plant run to a point
    /// </summary>
    
    public Node Node_PlantTo(Val<Vector3> targ)
    {
        Func<RunStatus> PlantTo =
            delegate ()
            {
                Vector3 targPos = targ.Value;
                Vector3 offet = Vector3.Normalize(Vector3.Cross(targPos, Vector3.up));
                return RunStatus.Success;
            };
        return new LeafInvoke(
            PlantTo,
